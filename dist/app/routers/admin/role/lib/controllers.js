...
const controllers = {
    createRole: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (data.aPermission?.length) {
            ...
            if (!permissions.length)
                return res.reply(messages.not_found('Permissions'));
            const isValid = permissions.every(({ sKey }) => aPermission.includes(sKey));
            if (!isValid)
                return res.reply(messages.bad_permissions());
        }
        const role = await Roles.findOne({ sName: data.sName });
        if (role)
            return res.reply(messages.already_exists('Role'));
    ...
    listRoles: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        const query = { eStatus: { $ne: eStatus.D } };
        if (search)
        ...
        const roles = await Roles.find(query, { projection: { dCreatedDate: false, dUpdatedDate: false } })
            ...
            .toArray();
        if (!roles.length)
            return res.reply(messages.not_found('Roles'));
    ...
    editRole: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (error)
            return res.reply(messages.unprocessableEntity(), _.parse(error));
        const updateObj = { $set: { dUpdatedDate: data.dUpdatedDate }, $pull: {} };
        if (data.aPermission?.length) {
            const aPermission = data.aPermission.map(e => e.ePermission);
            const permissions = await Permissions.find({}).toArray();
            if (!permissions.length)
        ...
        if (!role)
            return res.reply(messages.not_found('Role'));
        if (data.sName) {
        ...
        if (data.eStatus)
            updateObj.$set.eStatus = data.eStatus;
    ...
    createPermission: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!body.sKey || typeof body.sKey != 'string')
            return res.reply(messages.unprocessableEntity(), { path: 'sKey', message: 'Must be a string!' });
        const checkExists = await Permissions.findOne({ sKey: body.sKey });
        if (checkExists)
            return res.reply(messages.already_exists('Permission'));
        ...
        if (roles.length)
            await Roles.updateMany({}, { $push: { aPermissions: { sKey: body.sKey, eType: ePermissionType.N } } });
