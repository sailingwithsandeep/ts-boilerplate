...
const controllers = {
    ...
    changePassword: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!req.body.sPassword)
            return res.reply(messages.required_field('current password'));
        if (!req.body.sNewPassword)
            return res.reply(messages.required_field('new Password'));
        if (!_.comparePassword(req.body.sPassword, req.user.sPassword))
            return res.reply(messages.wrong_password());
        if (req.body.sPassword === req.body.sNewPassword)
