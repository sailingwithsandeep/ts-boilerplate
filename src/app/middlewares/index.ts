...
const middlewares = {
    isAdminAuthenticated: _.wrapWithTryCatchAsync(async (req, res, next) => {
        ...
        if (!user) return res.reply(messages.unauthorized());
        if (user.eStatus === 'D') return res.reply(messages.custom.admin_deleted);
        if (user.eStatus === 'N') return res.reply(messages.custom.admin_blocked);
