...
const controllers = {
    createAdmin: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (error) return res.reply(messages.unprocessableEntity(), error);
        if (_.isEmptyObject(data)) return res.reply(messages.server_error());
        ...
        if (adminExist && adminExist.sUsername === req.body.sUsername) return res.reply(messages.already_exists('Username'));
        if (adminExist && adminExist.sEmail === req.body.sEmail) return res.reply(messages.already_exists('Email'));
        if (adminExist && adminExist.sMobile === req.body.sMobile) return res.reply(messages.already_exists('Mobile'));
    ...
    login: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!admin) return res.reply(messages.wrong_credentials());
        if (!_.comparePassword(req.body.sPassword, admin.sPassword as string)) return res.reply(messages.wrong_credentials());
        if (admin.eStatus == eStatus.N) return res.reply(messages.custom.admin_deleted);
        if (admin.eStatus == eStatus.D) return res.reply(messages.custom.admin_deleted);
    ...
    forgotPassword: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!response) return res.reply(messages.server_error());
    ...
    verifyEmail: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!admin) return res.reply(messages.already_verified());
        if (admin?.sVerificationToken != req.params.token) return res.reply(messages.expired('Link'));
    ...
    resetPassword: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!params.token) return res.reply(messages.unauthorized());
    ...
    verifyToken: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!user) return res.reply(messages.expired('Link'));
