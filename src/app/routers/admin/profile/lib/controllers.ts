...
const controllers = {
    editProfile: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!admin) return res.reply(messages.not_found('Admin'));

        const updateObj: Record<string, any> = { $set: { dUpdatedDate: new Date() }, $pull: {} };

        const adminExist = await Admin.findOne({ $or: [{ sEmail: data.sEmail }, { sUserName: data?.sUsername }] });
        if (adminExist) {
            if (data?.sUsername && adminExist.sUsername === data.sUsername) return res.reply(messages.already_exists('Username'));
            if (data?.sEmail && adminExist.sEmail === data?.sEmail && data.sEmail !== admin.sEmail) return res.reply(messages.already_exists('Email'));
        }
        ...
        if (data.sUsername) updateObj.$set.sUsername = data.sUsername;
    ...
    updatePassword: _.wrapWithTryCatchAsync(async (req, res) => {
        ...
        if (!_.comparePassword(body.sPassword, req.admin.sPassword as string)) return res.reply(messages.wrong_password());
        ...
        await Admin.updateOne({ _id: admin._id }, { $pull: { aJwtTokens: req.token }, $set: { sPassword: _.encryptPassword(data.sPassword as string) } });
